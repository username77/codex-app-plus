...
function SettingsSelectMenu<T extends string>(props: {
  ...
  return (
    <div className="toolbar-split-menu settings-select-menu" role="menu" aria-label={props.label}>
...
export function SettingsSelectRow<T extends string>(props: SettingsSelectRowProps<T>): JSX.Element {
  ...
  return (
    <div className="settings-row">
      ...
      <div className="settings-row-control" ref={containerRef}>
        <button type="button" className="settings-select-trigger" aria-haspopup="menu" aria-expanded={menuOpen} aria-label={`${props.label}：${selectedOption.label}`} onClick={() => setMenuOpen((value) => !value)}>
        ...
        {menuOpen ? <SettingsSelectMenu label={props.label} options={props.options} value={props.value} onSelect={handleSelect} /> : null}
      </div>
    </div>
  );
}
